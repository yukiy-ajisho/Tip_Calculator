-- Migration: Create calculate_tips function v4 test version
-- Execute this in Supabase SQL Editor
-- This is a test version that returns results directly instead of inserting into tip_calculation_results table
-- Based on v4, but modified to return TABLE instead of inserting into database

CREATE OR REPLACE FUNCTION calculate_tips_v4_test(
...
COMMENT ON FUNCTION calculate_tips_v4_test(UUID, UUID) IS 
'Test version of calculate_tips v4 that returns results directly as TABLE instead of inserting into tip_calculation_results.
Based on v4, but modified to return TABLE(name TEXT, date DATE, tips NUMERIC, cash_tips NUMERIC).
Use this for testing in SQL Editor:
  SELECT * FROM calculate_tips_v4_test(''calculation-id'', ''store-id'');

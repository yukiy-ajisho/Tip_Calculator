-- Migration: Create calculate_tips function v5 test version
-- Execute this in Supabase SQL Editor
-- This is a test version that returns results directly instead of inserting into tip_calculation_results table
-- Based on v5, but modified to return TABLE instead of inserting into database
-- Version 5:
--   - No start_time_adjustment_minutes support (uses raw working hours)
--   - Returns results grouped by date (employee x date)

CREATE OR REPLACE FUNCTION calculate_tips_v5_test(
...
BEGIN
  ...
  -- Structure: { "employee_name": { "2025-11-01": { "tips": 0 }, "2025-11-02": { "tips": 0 } } }
  ...
  LOOP
    -- Calculate total working hours for that day (no start-time adjustment in v5)
    SELECT COALESCE(
      SUM(EXTRACT(EPOCH FROM (formatted_working_hours."end"::TIME - formatted_working_hours.start::TIME)) / 3600),
      0
    )
    INTO v_total_hours
    ...
    IF v_total_hours > 0 THEN
      ...
      -- Distribute cash tips based on working hours (no start-time adjustment in v5)
      FOR v_working_hour_record IN
        SELECT
  ...
  -- Step 7: Return results as TABLE (employee x date)
...
COMMENT ON FUNCTION calculate_tips_v5_test(UUID, UUID) IS
'Test version of calculate_tips v5 that returns results directly as TABLE instead of inserting into tip_calculation_results.
Based on v5, but modified to return TABLE(name TEXT, date DATE, tips NUMERIC, cash_tips NUMERIC).
Version 5 features:
  - No start_time_adjustment_minutes support (uses raw working hours).
Use this for testing in SQL Editor:
  SELECT * FROM calculate_tips_v5_test(''calculation-id'', ''store-id'');
Returns results grouped by date (employee x date).';

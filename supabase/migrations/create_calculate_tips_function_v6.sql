-- Migration: Create calculate_tips function (v6)
...
--   - All features from v5 remain unchanged
...
BEGIN
  ...
  -- Step 7: Insert results into tip_calculation_results table
  -- Aggregate by employee (sum across all dates) and use period_start as the date
  FOR v_employee_name IN SELECT jsonb_object_keys(v_employee_tips)
  LOOP
    -- Calculate total tips for this employee across all dates
    v_total_tips := 0;
    FOR v_date_str IN SELECT jsonb_object_keys(v_employee_tips->v_employee_name)
    LOOP
      v_employee_tips_for_date := COALESCE(((v_employee_tips->v_employee_name->v_date_str)->>'tips')::NUMERIC, 0);
      v_total_tips := v_total_tips + v_employee_tips_for_date;
    END LOOP;

    -- Calculate total cash tips for this employee across all dates
    v_total_cash_tips := 0;
    IF v_employee_cash_tips ? v_employee_name THEN
      FOR v_date_str IN SELECT jsonb_object_keys(v_employee_cash_tips->v_employee_name)
      LOOP
        v_employee_cash_tips_for_date := COALESCE(((v_employee_cash_tips->v_employee_name->v_date_str))::NUMERIC, 0);
        v_total_cash_tips := v_total_cash_tips + v_employee_cash_tips_for_date;
      END LOOP;
    END IF;

    -- Insert one record per employee with period_start as the date
    INSERT INTO tip_calculation_results (
      id,
      calculation_id,
      name,
      date,
      ...
      v_employee_name,
      v_period_start,
      TRUNC(v_total_tips, 2),
      TRUNC(v_total_cash_tips, 2)
    );
  END LOOP;

  -- Also insert employees who only have cash tips (no tips from payments)
  FOR v_employee_name IN SELECT jsonb_object_keys(v_employee_cash_tips)
  LOOP
    -- Check if this employee already exists in results
    IF NOT EXISTS (
      SELECT 1 
      FROM tip_calculation_results 
      WHERE calculation_id = p_calculation_id 
        AND name = v_employee_name
    ) THEN
      -- Calculate total cash tips for this employee across all dates
      v_total_cash_tips := 0;
      FOR v_date_str IN SELECT jsonb_object_keys(v_employee_cash_tips->v_employee_name)
  ...
  -- Step 8: Update tip_calculations status to 'completed'
...
EXCEPTION
  WHEN OTHERS THEN
    -- Update tip_calculations status to 'failed' on error
    UPDATE tip_calculations
    SET status = 'failed',
        updated_at = NOW()
    WHERE id = p_calculation_id;
...
COMMENT ON FUNCTION calculate_tips(UUID, UUID) IS 
...
Version 6: 
  ...
  - All other features from v5 remain unchanged (aggregation by employee, trainee-only distribution, status updates).
Returns VOID.';

-- Migration: Create calculate_tips function (v5_case_insensitive_employee_tip_status)
-- Execute this in Supabase SQL Editor
-- This function calculates tip distribution for employees based on working hours and tip payments
-- Version 5_case_insensitive_employee_tip_status: 
--   - Based on v5: Aggregates tips and cash_tips by employee (sum across all dates)
--   - Uses period_start as the date for all results
--   - One record per employee instead of one record per employee per date
--   - Case-insensitive role matching: Uses ILIKE instead of LIKE for role name matching
--   - Cash tips are only distributed to employees marked as "tipped" in employee_tip_status table
...
BEGIN
  ...
  LOOP
    ...
    FOR v_working_hour_record IN
      ...
      LEFT JOIN role_mappings rm ON (
        ...
        AND (
          (rm.actual_role_name IS NOT NULL AND wh.role ILIKE '%' || rm.actual_role_name || '%')
          OR (rm.trainee_role_name IS NOT NULL AND wh.role ILIKE '%' || rm.trainee_role_name || '%')
        )
      )
      ...
      IF v_role_mapping_exists AND v_working_hour_record.role_name IS NOT NULL THEN
        ...
        v_working_roles := v_working_roles || jsonb_set(
          ...
          jsonb_build_array(jsonb_build_object(
            ...
            'is_trainee', (v_working_hour_record.trainee_role_name IS NOT NULL AND v_working_hour_record.role ILIKE '%' || v_working_hour_record.trainee_role_name || '%'),
  ...
  LOOP
    -- Calculate total working hours for that day (only for tipped staff)
    ...
    WHERE formatted_working_hours.stores_id = p_store_id
      ...
      -- Only include employees who are marked as tipped
      AND EXISTS (
        SELECT 1 FROM employee_tip_status
        WHERE employee_tip_status.calculation_id = p_calculation_id
          AND employee_tip_status.employee_name = formatted_working_hours.name
          AND employee_tip_status.is_tipped = true
      );
    ...
    IF v_total_hours > 0 THEN
      ...
      -- Distribute cash tips based on working hours (only for tipped staff)
      FOR v_working_hour_record IN
        SELECT 
...
Version 5_case_insensitive_employee_tip_status: 
  ...
  - Case-insensitive role matching: Uses ILIKE instead of LIKE for role name matching.
  - Cash tips are only distributed to employees marked as "tipped" in employee_tip_status table.
  - All other features from v5 remain unchanged (trainee-only distribution, status updates).
Returns VOID.';

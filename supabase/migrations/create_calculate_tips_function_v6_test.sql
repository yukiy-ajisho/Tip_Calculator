-- Migration: Create calculate_tips function v6 test version
-- Execute this in Supabase SQL Editor
-- This is a test version that returns results directly instead of inserting into tip_calculation_results table
-- Based on v6, but modified to return TABLE instead of inserting into database
-- Version 6: 
--   - Adds start_time_adjustment_minutes support: adjusts start time during calculation
--   - Returns results grouped by date (employee x date)

CREATE OR REPLACE FUNCTION calculate_tips_v6_test(
...
DECLARE
  v_period_start DATE;
  v_period_end DATE;
  v_start_time_adjustment_minutes INTEGER;
  v_tip_record RECORD;
  ...
  v_date DATE;
  v_date_str TEXT;
  v_employee_tips_for_date NUMERIC;
  v_employee_cash_tips_for_date NUMERIC;
BEGIN
  ...
  -- Step 0.5: Retrieve start_time_adjustment_minutes from stores table
  SELECT COALESCE(start_time_adjustment_minutes, 0)
  INTO v_start_time_adjustment_minutes
  FROM stores
  WHERE id = p_store_id;

  -- Initialize employee tips tracking
  -- Structure: { "employee_name": { "2025-11-01": { "tips": 0, "cash_tips": 0 }, "2025-11-02": { "tips": 0, "cash_tips": 0 } } }
  ...
  FOR v_tip_record IN
    ...
    WHERE formatted_tip_data.stores_id = p_store_id
      AND formatted_tip_data.order_date >= v_period_start
      AND formatted_tip_data.order_date <= v_period_end
  ...
  LOOP
    ...
    FOR v_working_hour_record IN
      ...
      WHERE wh.stores_id = p_store_id
        ...
        AND v_tip_record.payment_time::TIME >= GREATEST(
          TIME '00:00:00',
          ((wh.date + wh.start::TIME) - (v_start_time_adjustment_minutes || ' minutes')::INTERVAL)::TIME
        )
    ...
    LOOP
      ...
      IF v_group_total > 0 THEN
        ...
        -- Distribute to each employee (by date)
        FOR v_employee_record IN SELECT value FROM jsonb_array_elements(v_working_roles->v_role_name)
        LOOP
          ...
          v_date_str := v_tip_record.order_date::TEXT;

          -- Initialize employee if not exists
          IF NOT (v_employee_tips ? v_employee_name) THEN
            v_employee_tips := v_employee_tips || jsonb_build_object(v_employee_name, '{}'::jsonb);
          END IF;

          -- Initialize date for employee if not exists
          IF NOT ((v_employee_tips->v_employee_name)::jsonb ? v_date_str) THEN
            v_employee_tips := v_employee_tips || jsonb_set(
              v_employee_tips,
              ARRAY[v_employee_name],
              (v_employee_tips->v_employee_name)::jsonb || jsonb_build_object(v_date_str, jsonb_build_object('tips', 0))
            );
          END IF;
          ...
          IF v_is_trainee THEN
            ...
            v_employee_tips_for_date := COALESCE(((v_employee_tips->v_employee_name->v_date_str)->>'tips')::NUMERIC, 0);
            v_employee_tips := v_employee_tips || jsonb_set(
              v_employee_tips,
              ARRAY[v_employee_name, v_date_str, 'tips'],
              to_jsonb(v_employee_tips_for_date + v_trainee_amount)
            );
          ELSE
            -- Non-trainee receives per_person_amount (remaining amount will be added later)
            v_employee_tips_for_date := COALESCE(((v_employee_tips->v_employee_name->v_date_str)->>'tips')::NUMERIC, 0);
            v_employee_tips := v_employee_tips || jsonb_set(
              v_employee_tips,
              ARRAY[v_employee_name, v_date_str, 'tips'],
              to_jsonb(v_employee_tips_for_date + v_per_person_amount)
            );
          END IF;
        END LOOP;
          ...
          LOOP
            ...
            IF NOT v_is_trainee THEN
              ...
              v_employee_tips := v_employee_tips || jsonb_set(
                v_employee_tips,
                ARRAY[v_employee_name, v_date_str, 'tips'],
                to_jsonb(v_employee_tips_for_date + v_non_trainee_amount)
              );
            END IF;
          END LOOP;
  ...
  -- Step 6: Process cash tips (by date)
  FOR v_cash_tip_record IN
    ...
    WHERE formatted_cash_tip.stores_id = p_store_id
      AND formatted_cash_tip.date >= v_period_start
      AND formatted_cash_tip.date <= v_period_end
  LOOP
    -- Calculate total working hours for that day (using adjusted start time)
    SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (
      formatted_working_hours."end"::TIME - 
      GREATEST(
        TIME '00:00:00',
        ((formatted_working_hours.date + formatted_working_hours.start::TIME) - (v_start_time_adjustment_minutes || ' minutes')::INTERVAL)::TIME
      )
    )) / 3600), 0)
    INTO v_total_hours
    ...
    IF v_total_hours > 0 THEN
      ...
      v_date_str := v_cash_tip_record.date::TEXT;

      -- Distribute cash tips based on working hours (using adjusted start time)
      FOR v_working_hour_record IN
        SELECT 
      ...
      LOOP
        ...
        -- Calculate hours using adjusted start time
        v_employee_hours := EXTRACT(EPOCH FROM (
          v_working_hour_record.end_time::TIME - 
          GREATEST(
            TIME '00:00:00',
            ((v_working_hour_record.date + v_working_hour_record.start_time::TIME) - (v_start_time_adjustment_minutes || ' minutes')::INTERVAL)::TIME
          )
        )) / 3600;
        
        -- Initialize employee if not exists
        IF NOT (v_employee_cash_tips ? v_employee_name) THEN
          v_employee_cash_tips := v_employee_cash_tips || jsonb_build_object(v_employee_name, '{}'::jsonb);
        END IF;

        -- Initialize date for employee if not exists
        IF NOT ((v_employee_cash_tips->v_employee_name)::jsonb ? v_date_str) THEN
          v_employee_cash_tips := v_employee_cash_tips || jsonb_set(
            v_employee_cash_tips,
            ARRAY[v_employee_name],
            (v_employee_cash_tips->v_employee_name)::jsonb || jsonb_build_object(v_date_str, 0)
          );
        END IF;

        -- Add cash tip (truncate to 2 decimal places)
        v_employee_cash_tips_for_date := COALESCE(((v_employee_cash_tips->v_employee_name->v_date_str))::NUMERIC, 0);
        v_employee_cash_tips := v_employee_cash_tips || jsonb_set(
          v_employee_cash_tips,
          ARRAY[v_employee_name, v_date_str],
          to_jsonb(TRUNC(v_employee_cash_tips_for_date + (v_cash_tip_per_hour * v_employee_hours), 2))
        );
      END LOOP;
    END IF;
  END LOOP;

  -- Step 7: Return results as TABLE instead of inserting into tip_calculation_results
  -- Process all employees and dates
  FOR v_employee_name IN SELECT jsonb_object_keys(v_employee_tips)
  LOOP
    FOR v_date_str IN SELECT jsonb_object_keys(v_employee_tips->v_employee_name)
    LOOP
      v_employee_tips_for_date := COALESCE(((v_employee_tips->v_employee_name->v_date_str)->>'tips')::NUMERIC, 0);
      v_employee_cash_tips_for_date := COALESCE(((v_employee_cash_tips->v_employee_name->v_date_str))::NUMERIC, 0);

      -- Return row instead of inserting
      RETURN QUERY SELECT
        v_employee_name::TEXT,
        v_date_str::DATE,
        TRUNC(v_employee_tips_for_date, 2),
        TRUNC(v_employee_cash_tips_for_date, 2);
    END LOOP;
  END LOOP;

  -- Also return employees who only have cash tips (no tips from payments)
  FOR v_employee_name IN SELECT jsonb_object_keys(v_employee_cash_tips)
  LOOP
    FOR v_date_str IN SELECT jsonb_object_keys(v_employee_cash_tips->v_employee_name)
    LOOP
      -- Check if this employee-date combination was already returned above
      -- (by checking if it exists in v_employee_tips)
      IF NOT (v_employee_tips ? v_employee_name) OR NOT ((v_employee_tips->v_employee_name)::jsonb ? v_date_str) THEN
        v_employee_cash_tips_for_date := COALESCE(((v_employee_cash_tips->v_employee_name->v_date_str))::NUMERIC, 0);
...
COMMENT ON FUNCTION calculate_tips_v6_test(UUID, UUID) IS 
'Test version of calculate_tips v6 that returns results directly as TABLE instead of inserting into tip_calculation_results.
Based on v6, but modified to return TABLE(name TEXT, date DATE, tips NUMERIC, cash_tips NUMERIC).
Version 6 features:
  - Adds start_time_adjustment_minutes support: adjusts start time during calculation by subtracting the specified minutes.
  - For example, if start_time_adjustment_minutes is 10, a start time of 10:00:00 will be treated as 09:50:00 in calculations.
  - This adjustment applies to both tip payment matching and cash tip hour calculations.
Use this for testing in SQL Editor:
  SELECT * FROM calculate_tips_v6_test(''calculation-id'', ''store-id'');
Returns results grouped by date (employee x date).';

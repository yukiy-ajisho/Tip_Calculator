...
/**
 * Convert time string (HH:MM:SS) to HH:MM for input values
 */
function formatTimeHHMM(timeString: string | null): string {
  if (!timeString) return "";
...
export function WorkingHoursEditTable({
  ...
  const renderTableRow = (
    ...
    const getInputValue = (field: "date" | "start" | "end" | "role") => {
      ...
      if (field === "start") return formatTimeHHMM(record.start) || "";
      if (field === "end") return formatTimeHHMM(record.end) || "";
      if (field === "role") return record.role || "";
      return "";
    };
      ...
      >
        ...
        <td className="px-3 py-2 whitespace-nowrap text-xs border-r border-gray-200">
          {isEditing && (!hasData("start") || shouldShowOrange) ? (
            ...
            >
              {formatDisplayTime(record.start, timeFormat) || ""}
            </span>
          )}
        </td>
        <td className="px-3 py-2 whitespace-nowrap text-xs border-r border-gray-200">
          {isEditing && (!hasData("end") || shouldShowOrange) ? (
            ...
            >
              {formatDisplayTime(record.end, timeFormat) || ""}
            </span>
          )}
        </td>
        <td className="px-3 py-2 whitespace-nowrap text-xs border-r border-gray-200">
